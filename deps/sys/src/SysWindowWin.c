...
void SysWindowSetPixelsScaled3(SysWindow* instance, uint32_t* pixels, int scale)
{
    if (!SysWindowExists(instance)) return;
...
void SysWindowSetPixels1(SysWindow* instance, void* pixels)
{
    ...
    uint8_t* _pixels = (uint8_t*)pixels;
    ...
    for (int x = 0; x < width; x++)
    {
        uint8_t value = _pixels[x + y * width];
...
void SysWindowSetPixels3(SysWindow* instance, void* pixels)
{
    ...
    uint32_t* pixels1 = (uint32_t*)pixels;
    uint32_t* pixels2 = (uint32_t*)instance->pixels;

    int width  = instance->width;
    int height = instance->height;

    int offset1 = 0;
    int offset2 = width * (height - 1);

    for (int y = 0; y < height; y++)
    {
        for (int x = 0; x < width; x++)
        {
            pixels2[offset2+x] = pixels1[offset1+x];
        }
        offset1 += width;
        offset2 -= width;
    }
    ...
    //     int y1 = y;
    //     int y2 = height - 1 - y;
    //     pixels2[x + y2 * width] = pixels1[x + y1 * width];
    // }
}
void SysWindowSetPixels64x64ScalePerfect(SysWindow* instance, void* pixels)
{
    ...
    int offsetx = (width  - duno2) / 2;
    int offsety = (height - duno2) / 2;

    for (int y = 0; y < 64; y++)
    for (int x = 0; x < 64; x++)
    {
        uint32_t pixel = pixels1[x + y * 64];
        int x2 = x * scale;
        int y2 = y * scale;
        for (int i = 0; i < scale; i++)
        for (int j = 0; j < scale; j++)
        {
            int _x = x2+i+offsetx;
            int _y = y2+j+offsety;
            instance->pixels[_x + (height - 1 - _y) * width] = pixel;
            // SysWindowSetPixel(instance, x2+i+offsetx, y2+j+offsety, pixel);
        }
    }
}
void SysWindowSetPixelsTest(SysWindow* instance, uint32_t* pixels)
{
    int pixelCount = instance->widthRem * instance->heightRem;
    memcpy((void*)instance->pixels, (void*)pixels, pixelCount);
}
